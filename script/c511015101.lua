...
function c511015101.filter3(c,e,tp,m,f,chkf)
	return c511015101.filter2(c,e,tp,m,f,chkf) and aux.IsMaterialListCode(c,47198668)
end
function c511015101.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		e1:SetCondition(c511015101.subcon)
		e1:SetReset(RESET_EVENT+EVENT_LEAVE_FIELD)
		e:GetHandler():RegisterEffect(e1)
		
		local res2=Duel.IsExistingMatchingCard(c511015101.filter3,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,chkf)
...
function c511015101.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	mg1:AddCard(e:GetHandler()) 
	sg1:Merge(Duel.GetMatchingGroup(c511015101.filter3,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,chkf))
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
	...
	if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
		...
		if sg1:IsContains(tc) and (sg2==nil or not sg2:IsContains(tc) or not Duel.SelectYesNo(tp,ce:GetDescription())) then
			if not aux.IsMaterialListCode(tc,47198668) then
