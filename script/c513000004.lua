...
function c513000004.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	while wbc do
		...
		if c:IsFaceup() and c:GetFlagEffect(code)==0 then
			...
			e1:SetOperation(c513000004.resetop)
...
function c513000004.codefilter(c,code)
	return c:GetOriginalCode()==code and c:IsAttribute(ATTRIBUTE_DARK)
end
function c513000004.resetop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(c513000004.filter,tp,LOCATION_GRAVE,0,nil,ATTRIBUTE_DARK)
	if not g:IsExists(c513000004.codefilter,1,nil,e:GetLabelObject():GetLabel()) or c:IsDisabled() then
