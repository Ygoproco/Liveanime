...
function c511003055.initial_effect(c)
	...
	e1:SetTarget(c511003055.eqtg)
	e1:SetOperation(c511003055.eqop)
	c:RegisterEffect(e1)
	--unequip
	...
	e2:SetTarget(c511003055.sptg)
	e2:SetOperation(c511003055.spop)
	c:RegisterEffect(e2)
	--destroy sub
	...
	e4:SetValue(c511003055.eqlimit)
	c:RegisterEffect(e4)
	--destroy
	...
	e5:SetTarget(c511003055.destg)
	e5:SetOperation(c511003055.desop)
	c:RegisterEffect(e5)
end
c511003055.old_union=true
function c511003055.repval(e,re,r,rp)
	return bit.band(r,REASON_BATTLE)~=0
end
function c511003055.eqlimit(e,c)
...
function c511003055.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c511003055.filter(chkc) end
	if chk==0 then return e:GetHandler():GetFlagEffect(67159705)==0 and Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(c511003055.filter,tp,LOCATION_MZONE,0,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local g=Duel.SelectTarget(tp,c511003055.filter,tp,LOCATION_MZONE,0,1,1,e:GetHandler())
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,g,1,0,0)
	e:GetHandler():RegisterFlagEffect(67159705,RESET_EVENT+0x7e0000+RESET_PHASE+PHASE_END,0,1)
end
function c511003055.eqop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=Duel.GetFirstTarget()
	if not c:IsRelateToEffect(e) or c:IsFacedown() then return end
	if not tc or not tc:IsRelateToEffect(e) or not c511003055.filter(tc) then
...
function c511003055.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c511003055.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
